...
class Dialogue {
  ...
  createDialogue() {
    ...
    dialogueElem.style.setProperty(
      "--accent-color",
      "var(--" + this.color + "-accent)"
    );
    dialogueElem.style.setProperty(
  ...
  displayOptions(dialogueElem, options, optionsConfig) {
    ...
    if (optionsConfig.timedQuestion && optionsConfig.timedQuestion > 0) {
      ...
      timerSpan.textContent =
        (optionsConfig.timedQuestion / 1000).toFixed(1) + "s";
    ...
    Array.from(options).forEach((option) => {
      ...
      optionElement.addEventListener("click", () => {
        if (
          ...
          optionElement.classList.contains("locked")
        ) {
          return;
        }
        ...
        gameModule.getStoryline().answerResponse(answer);
        gameModule.answerQuestion();

        Array.from(optionsList).forEach(
          (option) => (option.disabled = option != optionElement)
        );
      });
    });
    ...
    for (let i = 0; i < text.length; ) {
      ...
      if (currentChar == "<") {
        ...
        if (currentChar.includes("/")) {
          ...
          dialogueSpeed = parseInt(
            currentChar.match(/dialogue-speed=(\d+)/g)[0].match(/(\d+)/g)[0]
          );
        } else if (currentChar.match(/pause duration=(\d+)/g)) {
          duration += parseInt(
            currentChar.match(/pause duration=(\d+)/g)[0].match(/(\d+)/)[0]
          );
          i++;
          continue;
        }
  ...
  static typewriteDialogue(dialogueElem) {
    ...
    let dialogueSpeed =
      Dialogue.prototype.dialogueSpeed * (1 / gameModule.getSetting("speed"));
      ...
      if (currentChar == "<") {
        ...
        if (currentChar.includes("/")) {
          ...
          setTimeout(
            typeNextCharacter,
            parseInt(
              currentChar.match(/pause duration=("\d+")/g)[0].match(/(\d+)/g)[0]
            )
          );
          return;
        }
